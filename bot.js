...
function getOnlineGameDisclaimer() {
    return `\n\n⚠️ *تنبيه مهم:*
هذه لعبة أونلاين. أنا لست مسؤولاً عن:
• حظر حسابك إذا استخدمت نسخة معدلة
• أي مشاكل تحدث أثناء اللعب
• فقدان بياناتك أو تقدمك في اللعبة

💡 *نصيحة:* استخدم النسخة الأصلية من Google Play للعب بأمان.`;
}
...
async function downloadAPKWithAxios(packageName, appTitle) {
    ...
    if (isOnlineGame(packageName, appTitle)) {
        console.log(`🎮 لعبة أونلاين - استخدام apkeep للنسخة الأصلية...`);
        
        // Rate limiting for apkeep
        if (currentApkeepDownloads >= MAX_CONCURRENT_APKEEP) {
            console.log(`⏳ انتظار - ${currentApkeepDownloads} تحميلات apkeep جارية...`);
...
${INSTAGRAM_URL}${POWERED_BY}`;
            ...
            if (!isGroup && !isAdmin) {
                ...
                if (antiPrivateResult.action === 'block_private') {
                    ...
                    console.log(`🚫 رسالة خاصة من ${senderPhone} - تم حظره في الخاص فقط`);
                    return;
                ...
                try {
                    const sections = [{
                        ...
                        rows: cleanResults.map((app, idx) => ({
                            ...
                            description: app.developer ? `المطور: ${app.developer}` : ''
                        }))
                    }];
...
💡 جرب تطبيق آخر أصغر${POWERED_BY}` 
    ...
    try {
        ...
        console.log(`🖼️ Icon debug - appDetails.icon: ${appDetails.icon}, appIcon: ${appIcon}, iconToUse: ${iconToUse}`);
...
⏳ جاري تقسيم الملف إلى أجزاء...${POWERED_BY}`
            ...
            addContext(userId, `❌ فشل تحميل التطبيق "${appTitle}". المستخدم لم يستلم الملف.`);
        }
